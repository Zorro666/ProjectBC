...
class RaceLogic {
public
  delegate int CubesRemainingInBagDelegate();
public
  delegate CupCardCubeColour NextCubeDelegate();
public
  delegate void AddCubeToPlayerDelegate(Player winner, CupCardCubeColour cube);
public
  delegate void DiscardCardDelegate(Card card);
public
  delegate void FinishRaceDelegate(Player winner, RaceLogic race);
...
public
  void SetPlayCardButtons(Card card) {
    var cardColour = (int)card.Colour;
    for (var playerIndex = 0; playerIndex < GameLogic.PlayerCount;
         ++playerIndex) {
      var cardIndex = m_cardsPlayed[playerIndex];
...
public
  ...
  string Name {
    get { return "Race" + NumberOfCubes.ToString(); }
  }
...
public
  void Initialise(int numberOfCubes, RaceUI raceUI,
...
public
  bool PlayCard(Player player, Card card, Player currentPlayer) {
    // Debug.Log(Name + " PlayCard " + player + " " + card.Colour + " " +
    // card.Value);
    if (State == RaceState.Finished) {
