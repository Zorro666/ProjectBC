...
public
  void SetCupStatus(int cupIndex, bool available) {
    m_unclaimedCupGOs[cupIndex].SetActive(available);
  }

public
  void SetCupInteractible(int cupIndex, bool interactible) {
    m_unclaimedCupButtons[cupIndex].interactable = interactible;
  }

  // TODO: this should be in logic not UI
public
  bool IsCupInteractible(int cupIndex) {
    return m_unclaimedCupButtons[cupIndex].interactable;
  }

public
  void SetPlayerCubeCountColour(int playerIndex, int cubeIndex, Color colour) {
    m_playerCubeCountsTexts[playerIndex, cubeIndex].color = colour;
  }

public
  void SetPlayerCubeCountValue(int playerIndex, int cubeIndex, int cubeValue) {
    m_playerCubeCountsTexts[playerIndex, cubeIndex].text = cubeValue.ToString();
  }

public
  void SetPlayerWildcardCubeCountValue(int playerIndex, int cubeValue) {
    m_playerWildcardCubeCountTexts[playerIndex].text = cubeValue.ToString();
  }

public
  void SetPlayerHandActive(int playerIndex, bool active) {
    m_playerHandGOs[playerIndex].SetActive(active);
  }

public
  void SetPlayerCardHighlighted(int playerIndex, int cardIndex,
                                bool highlighted) {
    m_playerCardOutlines[playerIndex, cardIndex].color =
        highlighted ? Color.white : Color.black;
  }

public
  void SetPlayerCard(int playerIndex, int cardIndex, Card card, Color colour) {
    string text = card.Value.ToString();
    m_playerCardValues[playerIndex, cardIndex].text = text;
    var textColour =
        (card.Colour == CupCardCubeColour.Yellow) ? Color.black : Color.white;
    m_playerCardValues[playerIndex, cardIndex].color = textColour;
    m_playerCardBackgrounds[playerIndex, cardIndex].color = colour;
  }

public
  void SetPlayerCup(int playerIndex, int cupDisplayIndex,
                    CupCardCubeColour cupType, int value, Color colour) {
    ...
    var textColour =
        (cupType == CupCardCubeColour.Yellow) ? Color.black : Color.white;
    m_playerCupValues[playerIndex, cupDisplayIndex].color = textColour;
  }

public
  void SetPlayerCupActive(int playerIndex, int cupIndex, bool active) {
    m_playerCupGOs[playerIndex, cupIndex].SetActive(active);
  }

public
  void ShowPlayerGenericButton(int playerIndex) {
...
public
  void SetPlayerGenericButtonText(int playerIndex, string text) {
    m_playerGenericButtons[playerIndex].GetComponentInChildren<Text>().text =
        text;
  }
  ...
  void Awake() {
    ...
    m_playerCubeCountsTexts =
        new Text[GameLogic.PlayerCount, GameLogic.CubeTypeCount];
    ...
    m_playerCardBackgrounds =
        new Image[GameLogic.PlayerCount, GameLogic.HandSize];
    m_playerCardValues = new Text[GameLogic.PlayerCount, GameLogic.HandSize];
    m_playerCupGOs =
        new GameObject[GameLogic.PlayerCount, GameLogic.MaxCupsPerPlayer];
    m_playerCupImages =
        new Image[GameLogic.PlayerCount, GameLogic.MaxCupsPerPlayer];
    m_playerCupValues =
        new Text[GameLogic.PlayerCount, GameLogic.MaxCupsPerPlayer];
...
public
  void Initialise() {
    ...
    for (var cupIndex = 0; cupIndex < GameLogic.CubeTypeCount; ++cupIndex) {
      ...
      if (m_unclaimedCupGOs[cupIndex] == null)
        Debug.LogError("Can't find cup " + (CupCardCubeColour)cupIndex + " " +
                       unclaimedCupName);
      m_unclaimedCupButtons[cupIndex] =
          m_unclaimedCupGOs[cupIndex].GetComponent<Button>();
    ...
    for (var player = 0; player < GameLogic.PlayerCount; ++player) {
      ...
      if (m_playerHandGOs[player] == null)
        Debug.LogError("Can't find Player Hand UI GameObject " +
                       (Player)player + " " + playerHandRootName);
      ...
      if (m_playerGenericButtons[player] == null)
        Debug.LogError("Can't find GenericButton for Player " + (Player)player +
                       " " + playerGenericButtonName);

      var playerCupsRootName = playerUIRootName + "CupsBackground/";
      for (var cupIndex = 0; cupIndex < GameLogic.MaxCupsPerPlayer;
           ++cupIndex) {
        ...
        if (m_playerCupGOs[player, cupIndex] == null)
          Debug.LogError("Can't find Cup " + playerCupIndex +
                         " Image GameObject " + cupImageName);

        m_playerCupImages[player, cupIndex] =
            m_playerCupGOs[player, cupIndex].GetComponent<Image>();
      ...
      for (var card = 0; card < GameLogic.HandSize; ++card) {
        ...
        var playerCardRootName =
            playerHandRootName + "Card" + cardIndex.ToString() + "/";

        var playerCardOutlineGO = GameObject.Find(playerCardRootName);
        if (playerCardOutlineGO == null)
          Debug.LogError("Can't find PlayerCardOutlineGO " +
                         playerCardRootName);
        m_playerCardOutlines[player, card] =
            playerCardOutlineGO.GetComponent<Image>();
        if (m_playerCardOutlines[player, card] == null)
          Debug.LogError("Can't find Player " + (Player)player + " Card[" +
                         cardIndex + " Outline Image " + playerCardRootName);
        ...
        if (playerCardBackgroundGO == null)
          Debug.LogError("Can't find PlayerCardBackgroundGO " +
                         playerCardBackgroundName);
        m_playerCardBackgrounds[player, card] =
            playerCardBackgroundGO.GetComponent<Image>();
        ...
        m_playerCardValues[player, card] =
            playerCardValueGO.GetComponent<Text>();
        if (m_playerCardValues[player, card] == null)
          Debug.LogError("Can't find Player " + (Player)player + " Card[" +
                         cardIndex + " Value Text " + playerCardValueName);
      }
      ...
      for (var cubeType = 0; cubeType < GameLogic.CubeTypeCount; ++cubeType) {
        var cubeCountText =
            playerCubesBackgroundRootName + (CupCardCubeColour)cubeType;
        var cubeCountGO = GameObject.Find(cubeCountText);
        if (cubeCountGO == null)
          Debug.LogError("Can't find " + (CupCardCubeColour)cubeType +
                         " cube count GameObject " + cubeCountText);
        m_playerCubeCountsTexts[player, cubeType] =
            cubeCountGO.GetComponent<Text>();
        if (m_playerCubeCountsTexts[player, cubeType] == null)
          Debug.LogError("Can't find " + (CupCardCubeColour)cubeType +
                         " cube count UI Text Component");
      }
      ...
      if (wildcardCubeCountGO == null)
        Debug.LogError("Can't find wildcard cube count GameObject " +
                       wildcardCubeCountText);

      m_playerWildcardCubeCountTexts[player] =
          wildcardCubeCountGO.GetComponent<Text>();
  ...
  bool Validate() {
    ...
    for (int playerIndex = 0; playerIndex < GameLogic.PlayerCount;
         playerIndex++) {
      if (m_playerWildcardCubeCountTexts[playerIndex] == null) {
        Debug.LogError("m_playerWildcardCubeCountTexts[" + playerIndex +
                       "] is null");
        return false;
      }
      ...
      for (int cubeIndex = 0; cubeIndex < GameLogic.CubeTypeCount;
           cubeIndex++) {
        if (m_playerCubeCountsTexts[playerIndex, cubeIndex] == null) {
          Debug.LogError("m_playerCubeCountsTexts[" + playerIndex + "," +
                         cubeIndex + "] is null");
          return false;
        }
      }
      for (var card = 0; card < GameLogic.HandSize; ++card) {
        if (m_playerCardOutlines[playerIndex, card] == null) {
          Debug.LogError("m_playerCardOutlines[" + playerIndex + "," + card +
                         "] is null");
          return false;
        }
        if (m_playerCardBackgrounds[playerIndex, card] == null) {
          Debug.LogError("m_playerCardBackgrounds[" + playerIndex + "," + card +
                         "] is null");
          return false;
        }
        if (m_playerCardValues[playerIndex, card] == null) {
          Debug.LogError("m_playerCardValues[" + playerIndex + "," + card +
                         "] is null");
          return false;
        }
      }
      for (var cupIndex = 0; cupIndex < GameLogic.MaxCupsPerPlayer;
        ...
        if (m_playerCupGOs[playerIndex, cupIndex] == null) {
          Debug.LogError("m_playerCupGOs[" + playerIndex + "," + cupIndex +
                         "] is null");
          return false;
        }
        if (m_playerCupImages[playerIndex, cupIndex] == null) {
          Debug.LogError("m_playerCupImages[" + playerIndex + "," + cupIndex +
                         "] is null");
          return false;
        }
        if (m_playerCupValues[playerIndex, cupIndex] == null) {
          Debug.LogError("m_playerCupValues[" + playerIndex + "," + cupIndex +
                         "] is null");
          return false;
        }
      }
