  ...
  void Start() {
    ...
    for (int player = 0; player < GameLogic.PlayerCount; ++player) {
      ...
      for (int j = 0; j < NumberOfCubes; ++j) {
        ...
        var playedCardsBackgroundGO =
            GameObject.Find(playedCardsBackgroundName);
        if (playedCardsBackgroundGO == null)
          Debug.LogError("Can't find PlayedCardsBackgroundGO " +
                         playedCardsBackgroundName);
        m_playedCardsBackground[player, j] =
            playedCardsBackgroundGO.GetComponent<Image>();
        ...
        if (playedCardsValueGO == null)
          Debug.LogError("Can't find PlayedCardsValueGO " +
                         playedCardsValueName);
...
public
  void SetPlayCardButtons(int playerIndex, int cardIndex, bool active) {
    SetPlayCardButtonInteractable(playerIndex, cardIndex, active);
    m_playedCardsGO[playerIndex, cardIndex].SetActive(active);
  }

public
  void SetPlayCardButtonInteractable(int playerIndex, int cardIndex,
                                     bool interactable) {
    m_playedCardsGO[playerIndex, cardIndex]
        .GetComponent<Button>()
...
public
  void StartRace(RaceState raceState) {
    for (var playerIndex = 0; playerIndex < GameLogic.PlayerCount;
         ++playerIndex) {
...
public
  void SetCube(int index, CupCardCubeColour colour) {
    m_cubeImages[index].color = m_gameLogic.GetCardCubeColour(colour);
  }

public
  void Initialise(GameLogic gamelogic) { m_gameLogic = gamelogic; }
