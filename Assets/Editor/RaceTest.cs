...
class RaceTest {
  ...
  int GetCubesRemainingInBag() { return m_numCubesInBag; }

  CupCardCubeColour NextCube() { return CupCardCubeColour.Red; }
  ...
  RaceLogic CreateRace(int numberOfCubes) {
    ...
    RaceUI raceUI = null;
    raceLogic.Initialise(numberOfCubes, raceUI, GetCubesRemainingInBag,
                         NextCube, AddCubeToPlayer, DiscardCard, FinishRace);
    return raceLogic;
  }

  [Test] public void
      InitialiseSetsNumbersOfCubes([Values(1, 2, 3, 4)] int numberOfCubes) {
    var raceLogic = CreateRace(numberOfCubes);
    Assert.That(raceLogic.NumberOfCubes, Is.EqualTo(numberOfCubes));
  }

  [Test] public void
      InitialiseSetsWinnerToUnkown([Values(1, 2, 3, 4)] int numberOfCubes) {
    var raceLogic = CreateRace(numberOfCubes);
    Assert.That(raceLogic.Winner, Is.EqualTo(Player.Unknown));
  }

  [Test] public void
      TheFirstOddRacesAreLowest([Values(1, 3)] int numberOfCubes) {
    m_numCubesInBag = 10;
    var raceLogic = CreateRace(numberOfCubes);
    raceLogic.NewGame();
  ...
  [Test] public void
      TheFirstEvenRacesAreHighest([Values(2, 4)] int numberOfCubes) {
    m_numCubesInBag = 10;
    var raceLogic = CreateRace(numberOfCubes);
    raceLogic.NewGame();
  ...
  [Test] public void StartRaceSetsRaceToFinishedWhenNotEnoughCubesInBag(
      [Values(1, 2, 3, 4)] int numberOfCubes) {
    m_numCubesInBag = 0;
    var raceLogic = CreateRace(numberOfCubes);
    raceLogic.StartRace();
