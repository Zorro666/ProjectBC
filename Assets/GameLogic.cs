...
public
  Color RaceLowestColour;
public
  Color RaceHighestColour;
public
  Color RaceFinishedColour;
...
public
  int GetCubesRemainingCount() { return m_cubesRemainingCount; }

public
  void DiscardCard(Card card) { m_discardDeck.Enqueue(card); }

public
  Color GetCardCubeColour(CupCardCubeColour cardCubeType) {
...
public
  void PlayerGenericButtonClicked() {
    switch (m_turnState) {
...
public
  void PlayerHandCardClicked(GameObject source) {
    ...
    if (playerHandSource != currentPlayerHand)
      return;
    // Debug.Log("PlayerHandCardClicked source:" + source.name + "
    // greatgrandparent:" + source.transform.parent.parent.name);
    ...
    for (var i = 0; i < m_maxNumCardsToSelectFromHand; ++i) {
      if ((m_chosenHandCards[i] >= 0) &&
          (m_chosenHandCards[i] == newChosenCardIndex)) {
    ...
    if ((m_maxNumCardsToSelectFromHand > 1) &&
        (m_chosenHandCardCount == m_maxNumCardsToSelectFromHand))
      newChosenCardIndex = -1;

    // Select new card
    if (newChosenCardIndex >= 0) {
      SelectCard(m_currentPlayer, newChosenCardIndex);
      // If new card is selected move the selected cards along and add to
      // position 0
    ...
    // Debug.Log("Selected card Colour: " + card.Colour + " Value: " +
    // card.Value);
    switch (m_turnState) {
...
public
  void PlayCardButtonClicked(GameObject source) {
    if (m_state != GameState.InGame)
      return;
    ...
    bool validCard =
        m_races[raceNumber - 1].PlayCard(side, card, m_currentPlayer);
    if (!validCard) {
...
public
  void ClaimCupButtonClicked(GameObject source) {
    if (m_state != GameState.InGame)
      return;
    var cupName = source.name;
    Debug.Log("ClaimCup:" + source.name + " by " + m_currentPlayer);
    var cupType = CupCardCubeColour.Count;
    for (CupCardCubeColour c = CupCardCubeColour.Grey;
         c < CupCardCubeColour.Count; ++c) {
    ...
    if ((m_playerCubeCounts[playerIndex, cupIndex] +
         m_playerWildcardCubeCounts[playerIndex]) < cubeCountToWin) {
      Debug.LogError("ClaimCup:" + cupName + " can't win the cup");
      return;
    }
    var numWildCardsNeeded =
        cubeCountToWin - m_playerCubeCounts[playerIndex, cupIndex];
  ...
  void ExitStartingPlayerTurn() {
    ...
    if (PlayerCanPlayCardOnARace()) {
      ...
      m_turnState = TurnState.PickCardsFromHandToDiscard;
      SetPlayerGenericButtonText("Discard " + m_chosenHandCardCount +
                                 " Cards from Hand");
  ...
  void DiscardSelectedCardsFromHand() {
    ...
    for (var i = 0; i < m_chosenHandCardCount; ++i) {
      ...
      var card = m_playerHands[playerIndex, cardIndex];
      Debug.Log("Discard[" + i + "] Card Colour " + card.Colour + " Value " +
                card.Value);
      DiscardCard(card);
      PlayerDrawNewCard(m_currentPlayer, cardIndex);
      m_chosenHandCards[i] = -1;
...
public
  ...
  void Initialise() {
    ...
    if (CubesTotalCount != cubeIndex)
      Debug.LogError("cubes count not matching " + CubesTotalCount + " != " +
                     cubeIndex);
  ...
  void NewGame() {
    ...
    for (var player = 0; player < GameLogic.PlayerCount; ++player) {
      for (var cubeType = 0; cubeType < GameLogic.CubeTypeCount; ++cubeType) {
        m_gameUI.SetPlayerCubeCountColour(
            player, cubeType, GetCardCubeColour((CupCardCubeColour)cubeType));
  ...
  void AwardCupToPlayer(Player player, CupCardCubeColour cupColour) {
    ...
    if (IsCupWon(cupColour)) {
      Debug.LogError("Cup has already been won " + cupColour + " by " +
                     m_cupOwner[cupIndex]);
    }
    ...
    if (cupDisplayIndex < GameLogic.MaxCupsPerPlayer) {
      ...
      m_gameUI.SetPlayerCup(playerIndex, cupDisplayIndex, cupColour,
                            cubeCountToWin, colour);
    }
  }
  ...
  void UpdateStatus() {
    switch (m_turnState) {
    case TurnState.StartingPlayerTurn:
      m_gameUI.SetStatusText(m_currentPlayer +
                             " Player: Press Continue to Start Turn");
      break;
    case TurnState.PickCardFromHand:
    ...
    case TurnState.EndingPlayerTurn:
      m_gameUI.SetStatusText(m_currentPlayer +
                             " Player: Press Continue to Finish Turn");
      break;
    }
  }
  ...
  void CreateFullDeck() {
    ...
    allCardValues[(int)CupCardCubeColour.Blue] =
        new int[]{1, 3, 5, 7, 9, 11, 13};
    allCardValues[(int)CupCardCubeColour.Green] =
        new int[]{1, 2, 4, 6, 7, 8, 10, 12, 13};
    allCardValues[(int)CupCardCubeColour.Yellow] =
        new int[]{1, 2, 3, 5, 6, 7, 8, 9, 11, 12, 13};
    allCardValues[(int)CupCardCubeColour.Red] =
    ...
    for (var colour = 0; colour < GameLogic.CubeTypeCount; ++colour) {
      ...
      if (m_cubeCurrentCounts[colour] != cardValues.Length)
        Debug.LogError((CupCardCubeColour)colour +
                       " CardValues length does not match cubeCounts");
  ...
  void Awake() {
    ...
    m_playerCubeCounts =
        new int[GameLogic.PlayerCount, GameLogic.CubeTypeCount];
  ...
  bool ValidateCards() {
    // TODO: Verify every card is in draw deck or discard deck or player hand or
    // played on a race
    return false;
  }
  ...
  bool ValidateCups() {
    ...
    for (var cupType = 0; cupType < GameLogic.CubeTypeCount; ++cupType) {
      ...
      if (cupWinCount > 1) {
        Debug.LogError("Cup " + (CupCardCubeColour)cupType +
                       " Invalid cupWinCount " + cupWinCount);
        allOk = false;
      }
    }
    return allOk;
  ...
  bool ValidateCubes() {
    ...
    // TODO: validate player cube counts and cups won and cubes remaining and
    // wildcards
    return allOk;
  }
